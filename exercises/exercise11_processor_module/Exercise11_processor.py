...
class sample_processor_t(idaapi.processor_t):
    ...
    plnames = ['Exercise 11 processor module']
    ...
    instruc = [
        ...
        {'name': 'mov', 'feature': CF_CHG1 | CF_USE2},
        {'name': 'st', 'feature': CF_CHG1 | CF_USE2},
        {'name': 'ld', 'feature': CF_CHG1 | CF_USE2},
        {'name': 'ldi', 'feature': CF_CHG1 | CF_USE2},
        {'name': 'ldih', 'feature': CF_CHG1 | CF_USE2},
        ...
        {'name': 'br', 'feature': CF_STOP | CF_JUMP},
        {'name': 'beq', 'feature': CF_JUMP},
        {'name': 'bne', 'feature': CF_JUMP},
        {'name': 'bc', 'feature': CF_JUMP},
        {'name': 'bnc', 'feature': CF_JUMP}
    ]
    ...
    assembler = {
        ...
        'name': "Exercise 11 processor module bytecode assembler",
    ...
    def emu(self):
        ...
        if (flags & CF_STOP) == 0:
            ua_add_cref(0, self.cmd.ea + 4, fl_F)

        if flags & CF_CALL:
            ua_add_cref(0, self.cmd[0].addr, fl_CN)

        if flags & CF_JUMP:
            ua_add_cref(0, self.cmd[0].addr, fl_JN)
    ...
    def outop(self, op):
        ...
        if op.type == o_reg:
            ...
            OutValue(op, OOFW_32 | OOF_NUMBER)
        elif op.type == o_near:
    ...
    def ana(self):
        ...
        op_bytes = ua_next_long()
        opcode = op_bytes >> 24

        cmd = self.cmd  # type: insn_t
        cmd.itype = opcode

        if opcode in [0, 1]:
            op = cmd.Op1  # type: op_t
            op.type = o_reg
            op.reg = (op_bytes >> 16) & 0x1f
            op.dtype = dt_word
        elif opcode == 3:
            pass
        elif opcode in [4, 5, 6, 14]:
            op = cmd.Op1  # type: op_t
            op.type = o_reg
            op.reg = (op_bytes >> 16) & 0x1f
            op.dtyp = dt_word

            op = cmd.Op2  # type: op_t
            op.type = o_reg
            op.reg = (op_bytes >> 8) & 0x1f
            op.dtype = dt_word
            ...
            op = cmd.Op2  # type: op_t
            op.type = o_imm
            op.value = op_bytes & 0xffff
            op.dtype = dt_word

            if op.value & 0x8000 == 0x8000:
                op.value |= 0xFFFF8000 | op.value

        elif opcode in [9, 10, 11, 12, 13]:
            op = cmd.Op1  # type: op_t
            op.type = o_reg
            op.reg = (op_bytes >> 16) & 0x1f
            op.dtyp = dt_word
            ...
            op = cmd.Op3  # type: op_t
            op.type = o_reg
            op.reg = op_bytes & 0x1f
            op.dtype = dt_word

        elif opcode in [2, 15, 16, 17, 18, 19]:

            disp = op_bytes & 0xFFFFFF

            if disp & 0x800000 == 800000:
                disp |= 0xFF000000

            disp = (disp * 4) & 0xFFFFFFFF
            op = cmd.Op1  # type: op_t
            op.type = o_imm
            op.value = self.cmd.ea + 4 + disp
            op.dtype = dt_word
